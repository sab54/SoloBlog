...
router.get("/:id/email-accounts", (req, res, next) => {
    const userId = req.params.id;
    // Define the query to retrieve email accounts for the specified user
    const query = "SELECT * FROM email_accounts WHERE user_id = ?";
    ...
    global.db.all(query, [userId], (err, rows) => {
        if (err) {
            ...
            res.json(rows); // Change this line if you want to return JSON instead of rendering a view
        }
    });
});
